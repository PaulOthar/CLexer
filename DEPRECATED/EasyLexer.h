...
TokenArray* LexicalAnalisys(char *Input,TokenArray *Tokens,char *NumberTag,char *Numbers,char *UnknownTag,int SeparateUnknownBySpace){
		...
		if(Current_Size == 1){//if this is a new stream,Do:
			...
			if(Current_CharIsNumber){//If the current Char is a number,then this stream is also a number
				...
				if(foundKnown > 0){
					...
					if(*Current_Char != SpaceChar || !SeparateUnknownBySpace){
						Unknown_Stream[Unknown_Size] = *Current_Char;
						Unknown_Size++;
					}
					else if(Unknown_Size > 0 && SeparateUnknownBySpace){
